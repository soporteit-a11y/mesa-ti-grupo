...
export default async function CronogramasPage({ searchParams }: { searchParams: Record<string, string> }) {
  ...
  // Un colaborador sin permiso de edicion ve el cronograma pero no puede marcar
  // ninguna tarea. El admin siempre puede.
  ...
  // El colaborador solo ve los cronogramas de las empresas que tenga asignadas en
          ...
          Object.entries(groups).map(([company, g]) => (
            <section key={company} style={{ marginBottom: 28 }}>
              <div className="company-head">
                ...
                <span className="mono" style={{ color: "var(--muted)", fontSize: 12 }}>{g.items.length} cronograma(s)</span>
              </div>
              <div className="grid g2">
                {g.items.map((i) => {
                  ...
                  <article className="card init-card" key={i.id}>
                    ...
                    <TaskList initiativeId={i.id} tasks={i.tasks} locked={!esAdmin} readOnly={!puedeMarcar} />
